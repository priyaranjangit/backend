...
export class UsertypeComponent implements OnInit , OnDestroy {
  ...
  Submit() {
    switch (this.dbops) {
      case DBOperation.create:
        this._commonService.post(Global.BASE_API_PATH + "UserType/Save/", this.addForm.value).subscribe(res => {
          if (res.isSuccess) {
            this._toastr.success("Record has been saved successfully !!", "UserType Master");
            this.resetForm();
          } else {
            this._toastr.error(res.errors[0], "UserType Master");
          }
      ...
      case DBOperation.update:
        this._commonService.post(Global.BASE_API_PATH + "UserType/Update/", this.addForm.value).subscribe(res => {
          if (res.isSuccess) {
            this._toastr.success("Record has been updated successfully !!", "UserType Master");
            this.resetForm();
          } else {
            this._toastr.error(res.errors[0], "UserType Master");
          }
  ...
  Delete(_id: number) {
    ...
    Swal.fire({
      ...
      if (result.value) {
        this._commonService.post(Global.BASE_API_PATH + "UserType/Delete/", obj).subscribe(res => {
          if (res.isSuccess) {
            Swal.fire(
              'Deleted!',
