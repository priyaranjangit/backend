...
export class SizeComponent implements OnInit, OnDestroy {
  ...
  Submit() {
    switch (this.dbops) {
      case DBOperation.create:
        this._commonService.post(Global.BASE_API_PATH + "SizeMaster/Save/", this.addForm.value).subscribe(res => {
          if (res.isSuccess) {
            this._toastr.success("Record has been saved successfully !!", "Size Master");
            this.resetForm();
          } else {
            this._toastr.error(res.errors[0], "Size Master");
          }
      ...
      case DBOperation.update:
        this._commonService.post(Global.BASE_API_PATH + "SizeMaster/Update/", this.addForm.value).subscribe(res => {
          if (res.isSuccess) {
            this._toastr.success("Record has been updated successfully !!", "Size Master");
            this.resetForm();
          } else {
            this._toastr.error(res.errors[0], "Size Master");
          }
  ...
  Delete(_id: number) {
    ...
    // let name = this.objRows.find(x => x.id == _id).name;
    // let obj = {
    //   id: _id
    // };
    // this._commonService.post(Global.BASE_API_PATH + "SizeMaster/Delete/", obj).subscribe(res => {
    //   if (res.isSuccess) {
    //     this._toastr.success(`Size ${name} has been deleted successfully !!`, "Size Master");
    //     this.getData();
    //   } else {
    //     this._toastr.error(res.errors[0], "Size Master");
    //   }
